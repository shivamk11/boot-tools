...
mkdir -p out
./tools/mkbootimg --kernel kernel/zImage-dtb --ramdisk ramdisk.gz -o out/boot.img --base 0x00000000 --ramdisk_offset 0x01000000 --cmdline "console=ttyHSL0,115200,n8 androidboot.console=ttyHSL0 androidboot.hardware=qcom user_debug=31 msm_rtb.filter=0x37 vmalloc=400M utags.blkdev=/dev/block/platform/msm_sdcc.1/by-name/utags androidboot.write_protect=0 zcache" --pagesize 2048
sleep 2
setterm -bold 
rm ramdisk.gz
